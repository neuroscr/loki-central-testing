  ...
  serveTCP: function(port, callback) {
    ...
    var server=net.createServer(function (socket) {
      ...
      var client={
        ...
        send: function(str) {
          if (!client) {
            console.log('server connection', handle, 'is already closed, cant send', str);
            return 1;
          }
          if (!client.socket) {
            console.log('server connection', handle, 'socket is already closed, cant send', str);
            return 1;
          }
    ...
    server.letsClose=function(cb) {
      console.log('closing server', port, 'connections', clients.length);
      for(var i in server_clients) {
        console.log('destroying server', port, 'client', i);
      ...
      server_clients={};
      console.log('server', port, 'clients destroyed');
      server.close(function() {
        console.log('server port', port, 'closed');
        if (cb) cb();
      });
    }
    servers.push(server);
